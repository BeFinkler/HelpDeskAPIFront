...
export async function api(path, { method = 'GET', body, signal, anonymous = false } = {}) {
  ...
  try {
    ...
    if (!anonymous && token !== session.get())
      throw new DOMException('Sessão alterada.', 'AbortError');
    const payload = response.headers.get('content-type')?.includes('application/json')
      ? await response.json()
      : null;
    if (!response.ok) {
      if (response.status === 401 && !anonymous && token === session.get())
        window.dispatchEvent(new Event('helpdesk:unauthorized'));
      throw new ApiError(
        payload?.error?.message || 'O serviço está temporariamente indisponível. Tente novamente.',
        response.status,
        payload?.error?.code,
    ...
    if (!payload)
      throw new ApiError('O serviço retornou uma resposta inesperada. Tente novamente.');
    return payload;
  } catch (error) {
    if (error instanceof ApiError || signal?.aborted || error.name === 'AbortError') throw error;
    if (timeout.aborted)
      throw new ApiError(
        'O serviço está demorando para responder. Aguarde um momento e tente novamente.',
      );
    throw new ApiError(
