...
export default function AuthPage({ register = false }) {
  ...
  const submit = async (event) => {
    ...
    if (register && values.senha !== values.confirmacao)
      validation.confirmacao = 'As senhas precisam ser iguais.';
    ...
    try {
      if (register) {
        await api('/auth/cadastro', {
          method: 'POST',
          anonymous: true,
          body: { nome: values.nome.trim(), email: values.email.trim(), senha: values.senha },
        });
      ...
      setError(failure);
      setFields(
        Object.fromEntries((failure.fields || []).map((field) => [field.field, field.message])),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="auth-page">
      <section className="auth-story">
        <Link className="auth-brand" to="/login" aria-label="HelpDesk">
          <Logo light />
        </Link>
        <div className="auth-story-content">
          <span className="auth-kicker">
            <span /> SUA CENTRAL DE SUPORTE
          </span>
          <h1>
            Menos obstáculos.
            <br />
            Mais <span>soluções.</span>
          </h1>
          <p>
            Um espaço para conectar você à ajuda de que precisa. Simples, organizado e sempre a um
        ...
        <div className="auth-trust">
          <ShieldCheck size={17} /> Seu atendimento, do início à solução.
        </div>
      </section>
      <section className="auth-form-section">
        ...
        <div className="auth-form-wrap">
          <p className="eyebrow">{register ? 'SEU PRIMEIRO PASSO' : 'BOM TER VOCÊ AQUI'}</p>
          <h2>{register ? 'Crie sua conta' : 'Entre no seu espaço'}</h2>
          <p className="auth-description">
            {register
              ? 'Cadastre-se para abrir e acompanhar seus chamados.'
              : 'Acompanhe seus chamados e continue de onde parou.'}
          </p>
          ...
          <form onSubmit={submit} className="auth-form">
            {register && (
              <Field label="Nome completo" name="nome" error={fields.nome}>
                <input
                  id="nome"
                  name="nome"
                  autoComplete="name"
                  ...
                  required
                  disabled={busy}
                  aria-invalid={!!fields.nome}
                  aria-describedby={fields.nome ? 'nome-error' : undefined}
                />
              </Field>
            )}
            ...
            <Field
              ...
              name="senha"
              error={fields.senha}
              hint={register ? 'Pelo menos 8 caracteres.' : undefined}
            >
              <div className="password-input">
                <input
                  ...
                  placeholder={register ? 'Crie uma senha segura' : 'Digite sua senha'}
                  required
                  minLength={8}
                  disabled={busy}
                  aria-invalid={!!fields.senha}
                  aria-describedby={
                    fields.senha ? 'senha-error' : register ? 'senha-hint' : undefined
                  }
                />
                <button
                  type="button"
            ...
            {register && (
              <Field label="Confirme sua senha" name="confirmacao" error={fields.confirmacao}>
                <input
                  id="confirmacao"
                  name="confirmacao"
                  type={visible ? 'text' : 'password'}
                  ...
                  required
                  disabled={busy}
                  aria-invalid={!!fields.confirmacao}
          ...
          <div className="auth-form-footer">
            <ShieldCheck size={15} /> Um espaço seguro para o seu atendimento.
          </div>
        </div>
        <p className="auth-copyright">HelpDesk · Central de suporte</p>
      </section>
    </div>
  );
}
