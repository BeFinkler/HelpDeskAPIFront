...
export default function Tickets() {
  ...
  const filtered = ['busca', 'status', 'prioridade', 'categoria', 'meus'].some((key) =>
    params.has(key),
  );
  const submit = (event) => {
  ...
  return (
    <>
      <PageHeading
        eyebrow="ACOMPANHE CADA ETAPA"
        title={technician ? 'Central de chamados' : 'Meus chamados'}
        description={
          technician
            ? 'Organize a fila e encontre o próximo atendimento.'
            : 'Todas as suas solicitações, com o andamento sempre à vista.'
        }
      >
        {!technician && (
          <Link className="button button-primary" to="/chamados/novo">
            <Plus size={18} />
            Novo chamado
          </Link>
        )}
      ...
      <section className="panel">
        ...
        >
          <div className="filter-search">
            ...
            <input
              name="busca"
              type="search"
              aria-label="Buscar por título ou protocolo"
            ...
            {technician && (
              <label className="checkbox-label">
                <input
                  ...
                  name="meus"
                  value="true"
                  defaultChecked={params.get('meus') === 'true'}
                />
                Meus atendimentos
              </label>
            )}
        ...
        {loading ? (
          <Loading label="Buscando chamados..." />
        ) : error ? (
          ...
          <>
            <TicketTable tickets={result.data} technician={technician} />
            <Pagination
              meta={result.meta}
              onChange={(page) => {
          ...
          <EmptyState
            title={filtered ? 'Nenhum resultado encontrado' : 'Nenhum chamado por aqui'}
            description={
              filtered
